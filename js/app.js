...
function buildReportHTML(fromStr, toStr) {
  ...
  // Averages row for table footer
  const avgWaterStr = avgWater > 0 ? avgWater+'L' : '—';
  const avgWeightStr = weightDays.length ? +(weightDays.reduce((s,d)=>s+d.weight,0)/weightDays.length).toFixed(1)+'kg' : '—';
  const avgWorkoutPct = n > 0 ? Math.round(workoutCount/n*100)+'%' : '—';
  const avgsRowHTML = `<tr style="background:#f0f4f8;font-weight:700;border-top:2px solid #c0d0e0">
    <td style="color:#1e3a5f;font-size:0.65rem;letter-spacing:0.04em">AVG</td>
    <td>${avgCal || '—'}</td>
    <td>${avgPro || '—'}g</td>
    <td>${avgCarb || '—'}g</td>
    <td>${avgFat || '—'}g</td>
    <td>${avgWaterStr}</td>
    <td>${avgWeightStr}</td>
    <td style="text-align:center;font-size:0.7rem">${avgWorkoutPct}</td>
  </tr>`;
...
/* Bottom section grows to fill remaining height */
.page-fill{ flex:1; display:flex; flex-direction:column; }
.page-fill .bottom-row{ flex:1; align-items:stretch; }
.page-fill .bottom-row > div, .page-fill .bottom-row > .card{ display:flex; flex-direction:column; }
.page-fill .bottom-row > .card table{ flex:1; }
tfoot tr td{ font-size:0.72rem; }
...
@media print{
  ...
  tfoot tr{ background:#eef2f7!important; font-weight:700!important; border-top:1.5px solid #b0c4d8!important; }
  tfoot td{ font-size:6pt!important; }
